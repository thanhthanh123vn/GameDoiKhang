...
public class Player implements Cloneable {
	...
	private Image skill2; // Frame hiện tại
	...
	// Di chuyển nhân vật
	public void loadSkilss(String[] framePaths) {
		skills = new Image[framePaths.length];
		try {
			for (int i = 0; i < skills.length; i++) {
				skills[i] = new ImageIcon(framePaths[i]).getImage();
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	public Image loadSkil(int index) {
		return skills[index];
	}
